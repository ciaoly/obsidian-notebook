...
# 原文地址:[http://drops.wooyun.org/tips/4862](http://drops.wooyun.org/tips/4862)
...
本篇章中用到的隐写术的图片，都打包在了这里：[隐写术图片](../images_result/images/%E9%9A%90%E5%86%99%E6%9C%AF%E5%9B%BE%E7%89%87.zip)，想去自己尝试一遍的话可以去下载。
...
![](../images_result/images/2015021109583591324.png)
...
![](../images_result/images/2015021109583637323.png)

图 4.png

![](../images_result/images/2015021109583659244.png)
...
![](../images_result/images/2015021109583641426.png)

图 6.png

再解一下 qrcode，用在线的就可以[http://tool.chinaz.com/qrcode/，得到了 flag{AppLeU0}，如果是隐写的使用了 ascii 的话，可以使用 Stegsolve——Analyse——Data](http://tool.chinaz.com/qrcode/，得到了flag{AppLeU0}，如果是隐写的使用了ascii的话，可以使用Stegsolve——Analyse——Data) Extract 来查看 ascii 码。
...
用一个 ctf 的题目作为例子吧，iscc2014 中有一个题目，给了一个名为 此为 gif 图片. gif 的文件，打开发现了报错。有的时候，会需要我们去修复图片，这对我们对于图片的文件结构要有了解。找到 gif 的文件格式，然后对照这个破损的文件。Gif 的图片格式文档可以查看这个链接，[http://dev.gameres.com/Program/Visual/Other/GIFDoc.htm](http://dev.gameres.com/Program/Visual/Other/GIFDoc.htm)

![](../images_result/images/2015021109583717722.png)

图片 8.png

用 winhex 打开，我们会发现他和普通的 GIF 图片不一样，头部缺少了东西，在对比一些文档，会发现是少了 GIF8。

![](../images_result/images/2015021109583767535.png)

图片 9.png

我们手动修复一下，增加 GIF8。

![](../images_result/images/2015021109583779653.png)
...
![](../images_result/images/2015021109583742750.png)
...
[http://www.cnblogs.com/fengyv/archive/2006/04/30/2423964.html](http://www.cnblogs.com/fengyv/archive/2006/04/30/2423964.html)

有详细的介绍。

![](../images_result/images/2015021109583730998.png)

图片 12.png

可以用 pngcheck 来辅助我们观察，可以看得更加清晰。pngcheck.exe -v sctf.png

![](../images_result/images/2015021109583840003.png)

图片 13.png

可以看到，正常的块的 length 是在 65524 的时候就满了，而倒数第二个 IDAT 块长度是 45027，最后一个长度是 138，很明显最后一个 IDAT 块是有问题的，因为他本来应该并入到倒数第二个未满的块里。

![](../images_result/images/2015021109583813078.png)

图片 14.png

我们用 winhex 把这一部分异常的 IDAT 块给扣出来。然后就是要研究研究这个块是什么情况，发现了载体之后就是要想办法找出他的规律。观察那一部分的数据，可以看到是 16 进制的 78 9C 开头的，百度一下分析是 zlib 压缩的标志。在 png 的百度百科里也可以查到 PNG 的 IDAT 是使用从 LZ77 派生的无损数据压缩算法，可以用 zlib 解压。那么就尝试用 zlib 来解一下这段数据。Zlib 的扩展阅读[http://zlib.net/](http://zlib.net/)

我们使用 python 来编程，先把那段数据处理一下，保存成 16 进制的。

![](../images_result/images/2015021109583853090.png)
...
![](../images_result/images/2015021109583935376.png)
...
![](../images_result/images/2015021109583932639.png)

图片 17.png

![](../images_result/images/2015021109583959314.png)
...
![](../images_result/images/2015021109583938289.png)
...
![](../images_result/images/2015021109584058067.png)
...
![](../images_result/images/2015021109584098043.png)
...
![](../images_result/images/2015021109584194950.png)

图片 24.png

![](../images_result/images/2015021109584141205.png)
...
![](../images_result/images/2015021109584236818.png)

图片 26.png

JPK——binary——binary to ascii

![](../images_result/images/2015021109584252025.png)
...
这个总结其实还是缺很多的，因为隐写术能写的东西太多了，比如 jpg 的冗余信息的压缩也可以隐藏进信息，还有其他的多媒体文件也可以进行隐写，例如音频文件，视频文件等等，有很多东西可以研究。一开始是觉得隐写术特别的有趣才接触到的，就像是在藏宝寻宝一样，特别好玩，希望你们也可以感受到这种快乐。欢迎大家和我交流，我的博客地址是[http://appleu0.sinaapp.com/。](http://appleu0.sinaapp.com/。)

 [https://wooyun.js.org/drops/%E9%9A%90%E5%86%99%E6%9C%AF%E6%80%BB%E7%BB%93.html](https://wooyun.js.org/drops/%E9%9A%90%E5%86%99%E6%9C%AF%E6%80%BB%E7%BB%93.html)
